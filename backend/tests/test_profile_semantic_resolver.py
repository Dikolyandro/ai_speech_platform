...
class TestProfileSemanticResolver(unittest.TestCase):
    ...
    def test_ambiguity_no_context_skips_profile(self):
        columns_json = [
        ...
        q = "almaty"
        filters, dbg = resolve_profile_value_filters(query_text=q, columns_json=columns_json)
        self.assertEqual(len(filters), 0, dbg)

    def test_disambiguate_shared_level_by_column_words(self):
        columns_json = [
            {
            ...
            },
            {
                ...
                "semantic": {
                    "categorical_values_norm": ["low", "medium", "high"],
                    "tokens": ["access", "resources"],
                },
            },
        ]
        q = "parental involvement medium, access to resources high and exam score more than 70"
        filters, dbg = resolve_profile_value_filters(query_text=q, columns_json=columns_json)
        self.assertTrue(
            any(f.column == "Parental_Involvement" and f.value == "medium" for f in filters), dbg
        )
        self.assertTrue(
            any(f.column == "Access_to_Resources" and f.value == "high" for f in filters), dbg
        )
