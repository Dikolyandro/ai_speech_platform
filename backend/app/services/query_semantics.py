...
_ASR_LEXICAL_FIXES: list[tuple[str, str]] = [
    ...
    # Уровень образования родителей / школа → те же токены, что часто в CSV (латиница)
    (r"\bстарш(ая|ей|ие|их)?\s+школ\w*\b", "high school"),
]
...
_COLUMN_ALIASES_TO_CANON: dict[str, list[str]] = {
    ...
    "gender": ["пол", "гендер", "мужчина", "женщина", "парень", "девушка"],
    "exam_score": ["оценка", "балл", "балла", "баллов", "экзамен", "результат"],
    "distance_from_home": ["расстояние", "удалённость", "далеко", "близко", "от дома", "до дома"],
    "parental_education_level": ["образование родителей", "уровень образования", "колледж"],
    # Типичные имена колонок из CSV (PascalCase) — в col_set должно совпасть имя целиком
    "Gender": ["пол", "гендер", "мужчина", "женщина", "парень", "девушка"],
    "Exam_Score": ["оценка", "балл", "балла", "баллов", "экзамен", "результат"],
    "Distance_from_Home": ["расстояние", "удалённость", "от дома", "до дома"],
