...
def build_dynamic_filters_and_order(
    ...
    # --- column value (имя колонки как в схеме)
    for name in sorted(col_names, key=len, reverse=True):
        ln = name.lower()
        if name in skip_columns:
            continue
        esc = re.escape(ln)
        # равенство: channel online, city astana
        m = re.search(rf"\b{esc}\s+([a-zA-Zа-яА-ЯёЁ0-9_.-]+)\b", q)
        if m:
            v = _norm_val(m.group(1))
            if v and v.lower() != ln:
                if _is_numeric_sql_type(col_types.get(name, "")):
                    # числовая колонка: channel 12 — редко; попробуем как число
                    try:
                        add_cmp_numeric(name, "=", float(v.replace(",", ".")), "col_space_value_numeric")
                    except ValueError:
                        pass
                else:
                    add_eq_text(name, v, "col_space_value")
    ...
    # --- Сравнения для числовых колонок
    for name in col_names:
        ...
        for rx, op in (
            ...
            mm = re.search(rx, q)
            if mm:
                add_cmp_numeric(name, op, float(mm.group(1).replace(",", ".")), "numeric_cmp")
                break
